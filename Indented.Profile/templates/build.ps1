...
function TagRepository {
    ...
    $tagVersionString = Invoke-EcpNativeCommand git describe --tags --abbrev=0 -ErrorAction SilentlyContinue
    $tagVersion = $tagVersionString -replace '^v' -as [Version]

    $moduleManifest = Join-Path -Path $PSScriptRoot -ChildPath 'build\*' |
        Get-ChildItem -Filter '*.psd1' -Depth 2 |
        Where-Object { $_.Directory.Name -eq $_.BaseName -or $_.Directory.Parent.Name -eq $_.BaseName } |
        Select-Object -First 1 |
        Import-PowerShellDataFile

    $currentVersion = $moduleManifest['ModuleVersion'] -as [Version]

    if (-not $tagVersion -or $tagVersion -lt $currentVersion) {
        git tag ('v{0}' -f $currentVersion)
        git push --tags
    }
